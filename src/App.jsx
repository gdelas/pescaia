...
function MapaNautico({spot,modal}){
  ...
  useEffect(()=>{
    ...
    const map=L.map(mapRef.current,{
      center:[spot.lat,spot.lon],zoom:14,
    ...
    const mainIcon=L.divIcon({
      ...
      iconSize:[32,32],iconAnchor:[8,32],popupAnchor:[8,-32],className:"",
    });
    L.marker([spot.lat,spot.lon],{icon:mainIcon})
      .addTo(map)
      .bindPopup(`<b style="font-size:13px">${spot.n}</b><br/><small style="color:#666">${spot.r} · ${spot.t}</small>`);
    // Spots de pesca 🎣
    fishingSpots.forEach(fs=>{
      ...
      L.marker([fs.lat,fs.lon],{icon:fsIcon})
        .addTo(map)
        .bindPopup(`<b style="font-size:12px">${fs.name}</b><br/><small style="color:#666">${fs.desc}</small>`);
    });
    mapInst.current=map;
  },[spot]);
  ...
  return(
    <div style={{position:"relative",width:"100%",height:"100%"}}>
      ...
      {fishingSpots.length>0&&(
        <div style={{position:"absolute",top:8,left:8,zIndex:500,background:"rgba(255,255,255,.92)",backdropFilter:"blur(4px)",border:"1px solid rgba(30,95,160,.15)",borderRadius:5,padding:"3px 8px",fontSize:9,fontFamily:MN,color:AC}}>
          🎣 {fishingSpots.length} spots · toca para info
        </div>
      )}
    </div>
  );
...
// ─── CEBO CARD CON IMAGEN IA ──────────────────────────────────────────────────
function CeboCard({nombre, especie}){
  const[estado,setEstado]=useState("idle"); // idle | loading | ok | error
  const[datos,setDatos]=useState(null);
  const[modalOpen,setModalOpen]=useState(false);
  const cacheKey=`${nombre}__${especie}`;

  const buscarImagen=async()=>{
    if(estado==="loading")return;
    // Si ya está en caché, abrir directamente
    if(imageCache[cacheKey]){
      setDatos(imageCache[cacheKey]);
      setEstado("ok");
      setModalOpen(true);
      return;
    }
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          messages:[{
            role:"user",
            content:`Busca en internet una imagen real del señuelo o cebo de pesca llamado "${nombre}" usado para pescar ${especie} en España. 
Necesito encontrar una imagen de producto de una tienda online (Amazon España, Decathlon, tienda de pesca).
Devuelve SOLO un JSON con este formato exacto, sin texto adicional:
{
  "url_imagen": "URL directa a la imagen del producto (jpg/png)",
  "nombre_producto": "nombre exacto del producto",
  "tienda": "nombre de la tienda",
  "url_tienda": "URL de la página del producto",
  "descripcion": "descripción breve de 1 frase sobre cómo y cuándo usarlo para pescar ${especie}"
}
Si no encuentras imagen de producto real, devuelve url_imagen como null.`
          }],
        }),
      });
  ...
  return(
    <>
      {/* Píldora del cebo — toca para ver imagen */}
      <button
        ...
        >
          ...
          >
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              ...
              <button
                onClick={()=>setModalOpen(false)}
                style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:TXD,padding:"2px 6px"}}
              >×</button>
            </div>
            ...
            {estado==="loading"&&(
              <div style={{textAlign:"center",padding:"28px 0"}}>
                <div style={{fontSize:28,animation:"spin 1s linear infinite",display:"inline-block",color:AC}}>◌</div>
                <div style={{fontSize:11,color:TXD,marginTop:8,fontFamily:MN,letterSpacing:1}}>Buscando imagen...</div>
              </div>
            )}
            ...
            {estado==="ok"&&datos&&(
              <div>
                {/* Imagen */}
                {datos.url_imagen?(
                  <div style={{borderRadius:8,overflow:"hidden",marginBottom:10,background:"#f8fbff",border:`1px solid ${BD}`,aspectRatio:"4/3",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <img
                      src={datos.url_imagen}
                      alt={datos.nombre_producto||nombre}
                      style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain"}}
                      onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}}
                    />
                    <div style={{display:"none",flexDirection:"column",alignItems:"center",padding:20,color:TXD}}>
                      <div style={{fontSize:32,marginBottom:8}}>🪝</div>
                      <div style={{fontSize:11,textAlign:"center"}}>{nombre}</div>
                    </div>
                  </div>
                ):(
                  <div style={{borderRadius:8,background:"#f8fbff",border:`1px solid ${BD}`,padding:24,textAlign:"center",marginBottom:10}}>
                    <div style={{fontSize:40,marginBottom:8}}>🪝</div>
                    <div style={{fontSize:12,color:TXD}}>{nombre}</div>
                  </div>
                )}

                {/* Info producto */}
                {datos.nombre_producto&&(
                  <div style={{fontSize:11,fontWeight:600,color:TX,marginBottom:4}}>{datos.nombre_producto}</div>
                )}

                {/* Descripción uso */}
                {datos.descripcion&&(
                  <div style={{fontSize:11,color:TXM,lineHeight:1.55,background:"rgba(22,163,74,.05)",border:"1px solid rgba(22,163,74,.15)",borderRadius:6,padding:"7px 9px",marginBottom:8}}>
                    💡 {datos.descripcion}
                  </div>
                )}
                ...
                {datos.url_tienda&&(
                  <a
                    ...
                    style={{
                      ...
                      textAlign:"center",
                      padding:"9px",
                      background:"linear-gradient(135deg,#1e5fa0,#3b82f6)",
                      color:"#fff",
                      borderRadius:7,
                      fontSize:12,
                      fontWeight:600,
            ...
            {estado==="error"&&(
              <div style={{textAlign:"center",padding:"20px 0"}}>
                <div style={{fontSize:32,marginBottom:8}}>🪝</div>
                <div style={{fontSize:12,color:TX,fontWeight:600,marginBottom:4}}>{nombre}</div>
                <div style={{fontSize:11,color:TXD,marginBottom:12}}>No se encontró imagen disponible</div>
                <button
                  onClick={()=>{setEstado("idle");buscarImagen();}}
                  style={{padding:"7px 16px",borderRadius:6,cursor:"pointer",fontSize:12,background:AC,border:"none",color:"#fff"}}
                >
                  Reintentar
                </button>
              </div>
