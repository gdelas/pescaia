...
import{getFishingSpots,SPOT_ICONS}from"./data/fishingSpots";
...
// SVG pin estilo Google Maps / WhatsApp
const PIN_SVG=(color="#dc2626")=>`<svg xmlns="http://www.w3.org/2000/svg" width="28" height="38" viewBox="0 0 28 38">
...
const FISHING_PIN_SVG=`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="32" viewBox="0 0 24 32">
  <filter id="sh"><feDropShadow dx="0" dy="1" stdDeviation="1.5" flood-opacity="0.3"/></filter>
  <g filter="url(#sh)">
    <path d="M12 0C5.373 0 0 5.373 0 12c0 8 12 20 12 20S24 20 24 12C24 5.373 18.627 0 12 0z" fill="#1e5fa0"/>
    <circle cx="12" cy="12" r="5" fill="white" opacity="0.9"/>
    <text x="12" y="16" text-anchor="middle" font-size="7" fill="#1e5fa0">🎣</text>
  </g>
</svg>`;
...
function MapaNautico({spot}){
  ...
  const fishingSpots=useMemo(()=>spot?getFishingSpots(spot.n):[],[spot]);
  ...
  useEffect(()=>{
    ...
    // Pin principal del spot — aguja roja
    ...
    L.marker([spot.lat,spot.lon],{icon:mainIcon})
      .addTo(map)
      .bindPopup(`<b>${spot.n}</b><br/><small>${spot.r}</small>`);

    // Spots de pesca — pines azules
    fishingSpots.forEach(fs=>{
      const fsIcon=L.divIcon({
        html:FISHING_PIN_SVG,
        iconSize:[24,32],iconAnchor:[12,32],
        popupAnchor:[0,-32],className:"",
      });
      L.marker([fs.lat,fs.lon],{icon:fsIcon})
        .addTo(map)
        .bindPopup(`<b>${fs.name}</b><br/><small>${fs.desc}</small>`);
    });

    // Zoom controls simples
  ...
  return(
    <div style={{position:"relative",width:"100%",height:"100%"}}>
      ...
      {fishingSpots.length>0&&(
        <div style={{position:"absolute",top:6,right:6,zIndex:500,background:"rgba(255,255,255,.92)",backdropFilter:"blur(4px)",border:`1px solid ${BD}`,borderRadius:5,padding:"3px 7px",fontSize:9,fontFamily:MN,color:AC}}>
          🎣 {fishingSpots.length} spots de pesca
        </div>
      )}
    </div>
  );
